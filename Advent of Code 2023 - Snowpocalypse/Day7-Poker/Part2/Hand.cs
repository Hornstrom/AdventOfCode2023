...
public class Hand// : IComparable<Hand>
{
    ...
    private void CalculateType()
    {
        ...
        {
            numberOfInstancesOfCard[i] = CardsArray.Count(c => c.Equals(CardsArray[i]) && !c.Equals(1));
        }

        var jokers = CardsArray.Count(c => c.Equals(1));
        
        if (numberOfInstancesOfCard.Max() + jokers  == 5)
        {
            Type = 1;
        }
        else if (numberOfInstancesOfCard.Max() + jokers == 4)
        {
            Type = 2;
        }
        else if (numberOfInstancesOfCard.Max() == 3 && numberOfInstancesOfCard.Contains(2))
        {
            Type = 3;
        }
        else if (numberOfInstancesOfCard.Max() + jokers == 3 && numberOfInstancesOfCard.Count(c => c.Equals(2)) == 4)
        {
            Type = 3;
        }
        else if (numberOfInstancesOfCard.Max() + jokers == 3)
        {
            Type = 4;
        }
        ...
        else if (numberOfInstancesOfCard.Count(n => n == 2) == 2 || jokers == 1)
        {
            Type = 6;
        }
        else
